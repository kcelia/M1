...
def read_img(img):
    ...
    img(str): chemin de l'image a charger
    
    
    return (np.array (3D)): l'image en hsv sous forme de matrice 3D
...
def get_patches(img, size, step):
    ...
    img: image dont on veut extraire les patchs
    size: taille du patch
    step: pas utilisé pour extraire les patchs
    
    return tuple(list(np.array), list(np.array)): les listes des patchs morts et non morts
...
def noise_patch(patch, prc=.2):
    ...
    patch: patch dont on souhaite supprimer des pixels
    prc: pourcentage de perte des pixels
    
    return np.array (2D): le meme patch avec des pertes
...
def X2patch(L, h=H):
    ...
    L np.array : liste de valeurs a reconvertir en patch
    h int: hauteur du patch
    return np.array (3D): le patch en 3D
