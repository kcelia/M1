...
--MAP<Int,list<(String,String)>> map
--foreach(r:listeRowidsA){
--	t=Annuaire.getTuple(r)
--	map.put(t.cp,(t.nom, t.prenom)) #s'il y a deja un couple (nom,prenom)dans le cp alors
--			     		on ajoute le nouveau couple, on ne remplace pas le premier
--}
--foreach(v:Ville.fullScan()){
--	foreach( p:m.get(v.cp){  # on obtient les (nom, prenon) de la map correspondant à l'index cp 
--		 afficher( p.nom,p.prenom,pV.ville)
--	}
--}
--avec Annuaire est une TABLE
--     Ville est une TABLE
--     IndexAge est un INDEX
--     map est un MAP
--
...
--MAP<Int,String> map	 # map(cp,nomVille)
--foreach(v:Ville.fullScan){
--	map.put(v.cp,v.ville)
--}
--listeRowidsA = IndexAge.getRowID(18)
--foreach(r:listeRowidsA){
...
--}
--avec BigAnnuaire est une TABLE
--     Ville est une TABLE
--     IndexAge est un INDEX
--     map est une MAP
--Dans requête la table la table Ville est lue avant la table BigAnnuaire car oracle a choisit d'indexer les ville dans une table de hashage plutot que d'indexer les personnes dans table de hachage car il y a plus de personnes habitant dans une ville que de villes, donc ce serait trop couteux.
--
