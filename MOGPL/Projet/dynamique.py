...
def coloration(A, seq_r, seq_c):
    ...
    k = 0 # Condition d'arrêt pour les grilles non résolvables
    while (rows or cols) and k < 100:
        k += 1
        for i in rows:
            tmp = list(grille[i])
...
def propagation(filename, display=True):
...
if __name__ == '__main__':
    ...
    for i in range(0,17):
        start = clock()
        res = propagation("instances/"+str(i)+".txt", False)
        end = clock() - start
